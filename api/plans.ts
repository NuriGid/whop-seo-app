...
export default async function handler(req: any, res: any) {
    ...
    try {
        ...
        console.log(`🔐 Fetching plans for Company: ${targetCompanyId} (App Mode)`);

        // 4. CALL WHOP API VIA SDK
        // Docs: whop.plans.list({ company_id: '...' })
        const response = await whop.plans.list({
            company_id: targetCompanyId
        });

        const rawData = (response as any).data ? (response as any).data : response;
        const allPlans = Array.isArray(rawData) ? rawData : [];

        console.log(`📦 Fetched: ${allPlans.length} Plans`);

        return res.status(200).json({ data: allPlans });
