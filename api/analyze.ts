...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Ön kontrol isteği gelirse hemen "Tamam" de
  ...
  try {
    ...
    // 2. Doğrudan Google'a bağlan (Model: gemini-1.5-flash)
...
${prompt}` 
    ...
    // 3. Hata varsa yakala ve net bir şekilde söyle
    if (!aiResponse.ok) {
    ...
    // Hata neyse olduğu gibi ön yüze gönder (Artık 'null' yazmayacak)
